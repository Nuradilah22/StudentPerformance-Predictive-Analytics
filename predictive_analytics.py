...
"""Code diatas untuk membuat DataFrame kosong bernama models yang akan digunakan untuk menyimpan dan membandingkan akurasi (baik saat training maupun testing) dari tiga model yang akan dibangun
...
"""Code diatas untuk melatih model Logistic Regression menggunakan data training (X_train, y_train). Setelah model terlatih, akurasi model pada data training (X_train) dan data testing (X_test) dihitung menggunakan metode .score() dan hasilnya disimpan dalam DataFrame models pada baris 'train_acc' dan 'test_acc' di kolom 'LogisticRegression'.
...
"""Code ini melatih model Random Forest Classifier menggunakan data training (X_train, y_train). Setelah model terlatih, akurasi model pada data training (X_train) dan data testing (X_test) dihitung menggunakan metode .score() dan hasilnya disimpan dalam DataFrame models pada baris 'train_acc' dan 'test_acc' di kolom 'RandomForest'. Penggunaan random_state=42 memastikan hasil yang konsisten setiap kali model dilatih.
...
"""Code ini melatih model Gradient Boosting Classifier menggunakan data training (X_train, y_train). Setelah model terlatih, akurasi model pada data training (X_train) dan data testing (X_test) dihitung menggunakan metode .score() dan hasilnya disimpan dalam DataFrame models pada baris 'train_acc' dan 'test_acc' di kolom 'GradientBoosting'. Penggunaan random_state=42 memastikan hasil yang konsisten setiap kali model dilatih."""
...
- Akurasi keseluruhan model sebesar 74%.

- Performa terbaik muncul pada kelas GradeClass 4, dengan precision 0.87 dan recall 0.98, yang menunjukkan model sangat konsisten dalam mengidentifikasi siswa dengan nilai tertinggi.

- Kelas GradeClass 0 dan 1 memiliki performa rendah, terutama GradeClass 0 dengan nilai precision dan recall 0.00, mengindikasikan model kesulitan mengidentifikasi siswa dengan nilai terendah.
...
- Akurasi keseluruhan model mencapai 91%, tertinggi di antara ketiga model.

- Model menunjukkan distribusi prediksi yang seimbang pada semua kelas, terutama pada kelas GradeClass 2, 3, dan 4, dengan f1-score di atas 0.85.

- Meskipun kelas GradeClass 0 memiliki recall relatif rendah (0.41), precision-nya cukup tinggi (0.90), artinya model cenderung jarang salah mengklasifikasikan siswa ke kelas ini, tetapi juga sering melewatkannya.

##### Gradient Boosting – Evaluasi
"""
...
- Akurasi keseluruhan sebesar 90%.

- Performa klasifikasi merata pada hampir semua kelas, terutama GradeClass 1 sampai 4, dengan f1-score rata-rata di atas 0.83.

- Kelas GradeClass 0 berhasil dikenali lebih baik dibanding Logistic Regression, dengan f1-score 0.70.
